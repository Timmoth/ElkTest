    ...
    public async void DevicePerformsPostGetPutDeleteRequests()
    {
        ...
        var (device, api) = await _fixture.Setup(_output, new List<ApiEndpoint>
        {
            new()
            {
                Method = "POST",
                Path = "/api/user",
                RequestBody = JsonSerializer.Serialize(new { name = "Tim" }),
                ResponseBody = JsonSerializer.Serialize(new { id = 1 })
            },
            new()
            {
                Method = "GET",
                Path = "/api/user/1",
                ResponseBody = JsonSerializer.Serialize(new { name = "Tim" })
            },
            new()
            {
                Method = "PUT",
                Path = "/api/user/1",
                RequestBody = JsonSerializer.Serialize(new { name = "Tim1" })
            },
            new()
            {
                Method = "DELETE",
