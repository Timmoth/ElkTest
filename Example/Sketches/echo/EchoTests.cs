...
public class EchoTests : ElkTestBase
{
    ...
    public async void AnalogOutputPinEchosAnalogInputPin(int input, int expectedOutput)
    {
        ...
        // Act
        await device.SendAndWait(ElkDevice.Pins.Set(28, input));

        await Task.Delay(1000);

        // Assert
        var pinResponse = await device.SendAndWait(ElkDevice.Pins.Get(27));
    ...
    {
        //Arrange
        var (device, _) = await _fixture.Setup(_output); // Connect to the ElkTestDevice & reset the SUT

        //Act
        await device.SendAndWait(ElkDevice.Pins.Set(21, input)); // Set pin 21 high on the ElkTestDevice

        await Task.Delay(1000); // Arbitrary wait to ensure sut loop has executed

        //Assert
        var pinResponse = await device.SendAndWait(ElkDevice.Pins.Get(20)); // Read value from pin 20
        var actualValue = int.Parse(pinResponse.Arguments[0]);
